...
model.to(device)
model.load_state_dict(torch.load('../input/tmodel.bin'))
model.eval()

model2 = BERTBasedUncased()
model2.to(device)
model2.load_state_dict(torch.load('../input/tweetmode.bin'))
model2.eval()
...
class TweetDataset:
    ...
    def __getitem__(self, item):
        ...
        tok_tweet = self.tokenizer.encode(sequence=self.sentiment[item], 
        pari=tweet)
        ...
        padding_length = self.max_len - len(tok_token_ids)
...
df_test = pd.read_csv('../input/tweet-sentiment.csv')
...
with torch.no_grad():
    for bi, d in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        outputs_start2, outputs_end2 = model(
            ids=ids, 
            mask=mask, 
            token_type_ids=token_type_ids
        )
        ...
        fin_padding_lens.extend(padding_len.cpu().numpu().tolist())
        fin_tweet_tokens_ids.append(ids.cpu().detach().numpy().tolist())
...
fin_tweet_tokens_ids = np.vstack(fin_tweet_tokens)
jacacrds = []
threshold=0.3
for j in range(len(fin_tweet_tokens)):
    ...
    if padding_len > 0:
        mask_start = fin_ouputs_start[j, 3:-1][:-padding_len] >= threshold
        mask_end = fin_outputs_end[j, 3:-1][:-padding_len] >= threshold
        tweet_token_ids = fin_tweet_tokens_ids[j, 3:-1][:-padding_len]
    else:
        mask_start = fin_outputs_start[j, 3:-1] >= threshold
        mask_end = fin_outputs_end[j, 3:-1] >= threshold
        tweet_token_ids = fin_tweet_tokens_ids[j, 3:-1]
...
sample = pd.read_csv('../input/tweet-sentiment-sample-sumbission.csv')
